...
async def run_pipeline(query: str, mode: str = "fluid") -> dict:
    ...
    try:
        ...
        print(
            f"      Confidence: {reason_result.get('confidence', 0):.2f} | Evidence: {len(reason_result.get('evidence', []))}"
        )
        ...
        print(
            f"      Verdict: {verdict_result.get('verdict')} | Truth: {verdict_result.get('truth_score', 0):.3f} | W3: {verdict_result.get('tri_witness', 0):.3f}"
        )
        ...
        print(
            f"      Hash: {seal_result.get('content_hash', '')[:16]}... | Time: {seal_result.get('timestamp', 'N/A')}"
        )
