...
class PrecisionWeighter:
    ...
    def estimate_precision(
        ...
        return PrecisionEstimate(pi_likelihood=pi_L, pi_prior=pi_P, kalman_gain=K)

    def update_belief(
        self, current_confidence: float, evidence_confidence: float, precision: PrecisionEstimate
    ) -> float:
        """
...
def update_belief_with_precision(
    current_confidence: float, evidence_confidence: float, precision: PrecisionEstimate
) -> float:
    """Convenience function for precision-weighted belief update."""
    return _precision_weighter.update_belief(current_confidence, evidence_confidence, precision)
