...
class HardenedAnomalousContrastEngine:
    ...
    async def evaluate(
        ...
        if novelty > 0.8:
            reasoning.append(
                f"High novelty ({novelty:.2f}): Unprecedented query pattern (Jaccard {max_jaccard:.2f})"
            )
        elif novelty < 0.3:
            reasoning.append(
                f"Low novelty ({novelty:.2f}): Routine query (Jaccard {max_jaccard:.2f})"
            )
        ...
        if entropy_reduction > 0.8:
            reasoning.append(
                f"High clarity ({entropy_reduction:.2f}): Significant entropy reduction"
            )
        ...
        if ontological_shift > 0.8:
            reasoning.append(
                f"Major ontological shift ({ontological_shift:.2f}): Framework change detected"
            )
        ...
        if decision_weight > 0.8:
            reasoning.append(
                f"High stakes ({decision_weight:.2f}): Irreversible or multi-stakeholder"
            )
        ...
        if eureka_score >= EUREKA_THRESHOLD:
            verdict = "SEAL"
            reasoning.append(
                f"EUREKA Score {eureka_score:.2f} ≥ {EUREKA_THRESHOLD}: Permanent seal"
            )
