...
class EquilibriumFinder:
    ...
    def find_multiple_equilibria(self, n_samples: int = 100) -> List[EquilibriumPoint]:
        ...
        for _ in range(n_samples):
            ...
            random_state = {name: np.random.uniform(0.7, 1.0) for name in self.paradox_names}
        ...
        equilibria.sort(key=lambda e: e.trinity_score * e.balance_index, reverse=True)
    ...
    def analyze_equilibrium_landscape(self) -> Dict:
        ...
        return {
            ...
            "balance_index": {"mean": np.mean(balance_indices), "std": np.std(balance_indices)},
            "stability": {"mean": np.mean(stabilities), "std": np.std(stabilities)},
...
class PerturbationAnalyzer:
    ...
    def test_perturbation(
        ...
        perturbed = {k: max(0, min(1, v)) for k, v in perturbed.items()}
...
def demonstrate_equilibrium():
    ...
    perturbation = {"truth_care": -0.15, "clarity_peace": 0.05}  # Large drop in truth
    ...
    print(
        f"Average Trinity Score: {landscape['trinity_score']['mean']:.4f} ± {landscape['trinity_score']['std']:.4f}"
    )
    print(
        f"Score range: [{landscape['trinity_score']['min']:.4f}, {landscape['trinity_score']['max']:.4f}]"
    )
