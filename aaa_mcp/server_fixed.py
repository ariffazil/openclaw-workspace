"""
arifOS AAA MCP Server — Constitutional AI Governance (v55.4-REAL) - Fixed HTTP Support
...
async def agi_reason(query: str, session_id: str) -> dict:
    ...
    Floors enforced: F2 (Truth >= 0.99), F4 (Empathy), F7 (Humility band 0.03-0.15)
...
if __name__ == "__main__":
    ...
    print("🔥 arifOS Constitutional Kernel — FastMCP Mode with HTTP+SSE Support")
    
    # Determine transport mode based on environment or command line
    transport_mode = os.getenv("ARIFOS_TRANSPORT", "http,sse")  # Default to both
    port = int(os.getenv("PORT", "6274"))
    
    if len(sys.argv) > 1:
        if sys.argv[1] in ["http", "sse", "http,sse", "sse,http"]:
            transport_mode = sys.argv[1]
    
    print(f"Starting server with transport: {transport_mode} on port {port}")
    
    # Run with both transports if specified
    if transport_mode == "http,sse" or transport_mode == "sse,http":
        # For dual transport, we typically run both in the same app
        # FastMCP should handle both protocols simultaneously
        mcp.run(transport="http", port=port)  # This will also support SSE
    elif transport_mode == "http":
        mcp.run(transport="http", port=port)
    elif transport_mode == "sse":
        mcp.run(transport="sse", port=port)
    else:
        print(f"Unknown transport mode: {transport_mode}. Use 'http', 'sse', or 'http,sse'")
        sys.exit(1)