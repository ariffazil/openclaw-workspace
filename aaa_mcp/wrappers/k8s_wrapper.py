...
class ManifestParser:
    ...
    def parse(self, manifest: str) -> ManifestAnalysis:
        ...
        untrusted = [img for img in images 
                     if not any(trusted in img for trusted in self.TRUSTED_REGISTRIES)]
    ...
    def _calculate_f2_score(self, images: List[str], uses_digest: bool, 
                           untrusted: List[str]) -> float:
    ...
    def _calculate_f6_score(self, privileged: bool, run_as_root: bool,
                           host_network: bool, host_pid: bool,
                           has_limits: bool) -> float:
    ...
    def _calculate_f5_score(self, has_liveness: bool, has_readiness: bool,
                           strategy: Dict) -> float:
...
class K8sConstitutionalWrapper:
    ...
    async def evaluate_apply(
        ...
        floors.append({"floor": "F10", "name": "Ontology", "passed": f10_pass, 
        ...
        floors.append({"floor": "F2", "name": "Truth", "passed": f2_pass,
        ...
        floors.append({"floor": "F6", "name": "Empathy", "passed": f6_pass,
        ...
        floors.append({"floor": "F5", "name": "Peace²", "passed": f5_pass,
                      "score": analysis.f5_peace_score})
        
        # F1: Amanah (Reversibility)
        has_strategy = strategy in ["canary", "blue-green", "rolling"]
        f1_pass = has_strategy or namespace not in ["prod", "production"]
        floors.append({"floor": "F1", "name": "Amanah", "passed": f1_pass,
        ...
        if hard_failures:
            ...
            verdict = "888_HOLD"
            verdict_reason = "Production operation with critical blast radius requires human approval"
        else:
            verdict = "SEAL"
...
async def k8s_constitutional_delete(
    ...
    if result["verdict"] == "SEAL":
        ...
        result["message"] = "💎🧠 DITEMPA, BUKAN DIBERI 🔒 — Human approval required for prod delete"
