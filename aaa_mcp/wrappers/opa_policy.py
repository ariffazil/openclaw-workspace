        ...
        "checks": [
            {
                ...
                "severity": "critical",
                "check": lambda m: not m.get("spec", {}).get("template", {}).get("spec", {}).get("hostNetwork", False),
                "message": "Pod uses host network namespace",
            },
            {
                "name": "no_host_pid",
                "severity": "critical",
                "check": lambda m: not m.get("spec", {}).get("template", {}).get("spec", {}).get("hostPID", False),
        ...
        for doc in docs:
            ...
            for policy_name, policy_def in BUILT_IN_POLICIES.items():
                ...
                results.append(PolicyResult(
                    ...
                    passed=len(violations) == 0,
                    violations=violations,
                    severity="critical" if any(v.get("severity") == "critical" for v in violations) else "warning",
...
class F10OntologyValidator:
    ...
    def __init__(self):
        self.opa = OPAServerAdapter(
            url=os.environ.get("OPA_SERVER_URL", "http://localhost:8181")
        )
        ...
        for r in all_results:
            if isinstance(r.violations, list):
                all_violations.extend([
                    {"policy": r.policy_name, "violation": v}
