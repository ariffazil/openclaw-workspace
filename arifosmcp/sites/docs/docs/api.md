...
`arifOS_kernel` (legacy aliases: `arifOS.kernel`, `metabolic_loop_router`) executes an explicit `222_REALITY` policy stage between `333_MIND` and `666_HEART` for configured risk tiers, then forwards grounding status into `888_JUDGE` synthesis.
...
- **Prompts:** `arifos_kernel_prompt`, `search_reality_prompt`, `ingest_evidence_prompt`, `session_memory_prompt`, `audit_rules_prompt`, `check_vital_prompt`, `open_apex_dashboard`, `bootstrap_identity_prompt`.
