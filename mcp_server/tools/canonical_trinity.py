...
async def mcp_agi(
    ...
    if action_upper in ("SENSE", "THINK"):
        ...
        # Schema Alignment (v55.3)
        if "intent" not in result and "intent_lane" in result:
            result["intent"] = result["intent_lane"]
        if "lane" not in result and "intent_lane" in result:
            result["lane"] = result["intent_lane"]
