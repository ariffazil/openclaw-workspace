...
Load files in this exact order — each is mandatory before the first reply:

**Constitutional Foundation:**
1. `ROOT_CANON.yaml` — root file precedence and status manifest

**Expanded Identity Layer (from /root/AAA/IDENTITY/):**
2. `/root/AAA/IDENTITY/INFRA.md` — VPS location, container stack, access credentials
3. `/root/AAA/IDENTITY/CAPABILITIES.md` — full tool list (docker, git, python, web, telegram, filesystem)
4. `/root/AAA/IDENTITY/BOUNDARIES.md` — R0-R4 scope classes, irreversible triggers, 888_HOLD markers
...
**Workspace Identity:**
9. `SOUL.md` — your voice and style (also in hermes home as identity slot)
...
13. In direct/private chat, also read `MEMORY.md` if present
14. Read today and yesterday in `memory/` if present

**Earth-Domain Grounding:**
15. If the task touches Earth reasoning, GEOX, geology, petrophysics, wells, seismic, basin interpretation, or subsurface claims, ground through GEOX context before speaking confidently
...
If any file in the identity layer is missing, recreate it from `/root/AAA/IDENTITY/` immediately.
...
## Repo Routing Constitution (AMANAH)

**DITEMPA BUKAN DIBERI** — Routing intelligence is forged, not given.

Before any git write, commit, or push — route to the correct repo:

| Domain | Repo | VPS Path | GitHub |
|--------|------|----------|--------|
| Agent governance, routing policy, ADRs | AAA | `/root/AAA/` | `github.com/ariffazil/AAA` |
| Finance, portfolio, capital, macro | WEALTH | `/root/WEALTH/` | `github.com/ariffazil/wealth` |
| Earth, geo, wells, subsurface | GEOX | `/root/GEOX/` | `github.com/ariffazil/geox` |
| Constitutional kernel, F1–F13, MCP runtime | arifOS | `/root/arifOS/` | `github.com/ariffazil/arifOS` |
| Design, architecture, planning twin | A-FORGE | `/root/A-FORGE/` | `github.com/ariffazil/A-FORGE` |
| Web assets, public site | arif-sites | `/root/arif-sites/` | `github.com/ariffazil/arif-sites` |
| Personal/meta root | ariffazil | `/root/ariffazil/` | `github.com/ariffazil/ariffazil` |

**Routing rules:**
1. Finance code → WEALTH. MCP/floors → arifOS. Earth/geo → GEOX. Governance/routing → AAA. Design → A-FORGE.
2. If a file already lives in a repo, it stays there — never silently move it.
3. Cross-repo moves require explicit 888_HOLD human confirmation.
4. Confidence < 0.8 → STOP and ask Arif which repo.
5. Never `git push origin main` directly — always branch → PR → merge.

**Branch naming:** `{repo}/{short-description}` (e.g., `arifOS/floor-F07-fix`)
**Commit format:** `{domain}: {what changed} | {rationale}`

**Autonomous capability:** Read, branch, commit, open PR. **Requires human confirmation:** direct main push, cross-repo moves, deletion, branch protection changes.
...
See `REPO_ROUTING_CONSTITUTION.md` for full policy.
