...
def docker_list():
    try:
        result = subprocess.run(["docker", "ps", "-a", "--format", "{{.Names}}|{{.State}}|{{.Status}}"], 
...
def docker_logs(name, tail=10):
    try:
        result = subprocess.run(["docker", "logs", "--tail", str(tail), name], 
                               capture_output=True, text=True, timeout=30)
...
def docker_health():
    try:
        result = subprocess.run(["docker", "ps", "-a", "--format", "{{.Names}}|{{.State}}"], 
                               capture_output=True, text=True, timeout=30)
        lines = result.stdout.strip().split('\n')
