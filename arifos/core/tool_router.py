...
class CapabilityRouter:
    ...
    async def invoke(
        self, stage: str, capability_id: str, params: Dict[str, Any]
    ) -> InvocationResult:
        """
        ...
        try:
            ...
            if module.async_execution:
                result = await asyncio.wait_for(
                    self._execute_async(module, params), timeout=timeout
                )
            else:
                result = await asyncio.wait_for(self._execute_sync(module, params), timeout=timeout)
    ...
    def _record_failure(self, capability_id: str):
        ...
        threshold = (
            self.config.get("routing", {}).get("circuit_breaker", {}).get("failure_threshold", 5)
        )
    ...
    async def _log_invocation(self, stage: str, capability_id: str, result: Any, success: bool):
