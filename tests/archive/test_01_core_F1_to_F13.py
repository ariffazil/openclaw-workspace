...
def test_floor_exists_and_callable(floor_id: str, validator_name: str):
    ...
    assert hasattr(
        floor_validators, validator_name
    ...
    assert callable(validator), f"Floor validator {validator_name} is not callable"
...
def test_validate_all_floors_exists():
    ...
    assert hasattr(
        floor_validators, "validate_all_floors"
    ), "validate_all_floors not found in floor_validators"
    assert callable(floor_validators.validate_all_floors), "validate_all_floors is not callable"
...
def f6_empathy_split_benchmark() -> float:
    ...
    test_cases = [
        ...
        # Semantic-only case
        {"telemetry": None, "text": "I'm really struggling and need help", "expected_split": True},
    ]
