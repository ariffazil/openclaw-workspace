...
@pytest.mark.asyncio
async def test_fs_inspect_directory():
    """Test filesystem directory inspection."""
    result = await fs_inspect(path="/root/arifOS", max_depth=1)
    
    assert result.tool == "fs_inspect"
    assert result.status == "ok"
    assert result.data.get("root") == "/root/arifOS"
    assert "directories" in result.data
    assert "files" in result.data


@pytest.mark.asyncio
async def test_fs_inspect_pattern():
...
@pytest.mark.asyncio
async def test_log_tail_basic():
    """Test basic log tail functionality."""
    # Use a file we know exists
    test_log = "/var/log/syslog"
    if not Path(test_log).exists():
        pytest.skip(f"Log file {test_log} not available")
    
    result = await log_tail(log_path=test_log, lines=10)
...
async def test_log_tail_nonexistent():
    ...
    assert result.status == "error"
    assert "not found" in result.error.lower() or "Log file" in result.error


@pytest.mark.asyncio
async def test_log_tail_grep():
    """Test log tail with grep filter."""
    test_log = "/var/log/syslog"
    if not Path(test_log).exists():
        pytest.skip(f"Log file {test_log} not available")
    
    result = await log_tail(log_path=test_log, lines=100, grep_pattern=".*")
...
@pytest.mark.asyncio
async def test_config_flags_file():
    """Test config flags file inspection."""
    result = await config_flags(
        config_path="/root/arifOS/pyproject.toml",
        env_prefix=None
    )
...
async def test_forge_guard_critical_risk():
    ...
    assert result.status == "ok"
    assert result.data["verdict"] == "888_HOLD"
...
async def test_all_tools_latency():
    ...
    tools = [
        ...
        fs_inspect(path="/root", max_depth=1),
        ...
        forge_guard(action="test", target="/tmp", session_id="test", dry_run=True),
    ]
    
    results = await asyncio.gather(*tools)
    
    for result in results:
        assert result.latency_ms < 5000, f"{result.tool} took too long: {result.latency_ms}ms"
