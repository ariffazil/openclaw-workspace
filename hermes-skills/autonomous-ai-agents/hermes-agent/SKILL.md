...
- **Docker Compose wiring** (arifOS federation): `env_file: /root/AAA/.hermes/.env` must be set in compose; container restart wipes `docker cp` changes; always recreate after env_file changes. See `references/hermes-docker-compose-wiring.md`.
- **env_file is mandatory** for provider credentials: without it, the container runs but has zero API keys. Diagnose with `docker exec hermes-agent sh -c 'env | grep MINIMAX'`.
