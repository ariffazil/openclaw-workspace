...
class EquilibriumSolver:
    ...
    def solve(self, paradoxes: Dict[str, NineParadox]) -> EquilibriumState:
        ...
        convergence_delta = math.sqrt((self.threshold - trinity_score) ** 2 + std_dev**2)
...
class TrinityNine:
    ...
    def _determine_verdict(
        self, eq: EquilibriumState, alpha: float, beta: float, gamma: float
    ) -> str:
    ...
    def _generate_reasoning(self, eq: EquilibriumState, path: List[Dict]) -> str:
        ...
        if eq.is_equilibrium:
            ...
            parts.append(
                f"✗ Not in equilibrium (score: {eq.trinity_score:.3f}, need ≥{EQUILIBRIUM_THRESHOLD})"
            )
