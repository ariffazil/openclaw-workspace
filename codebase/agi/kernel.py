...
class AGINeuralCore:
    ...
    async def sense(self, query: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "lane": (
                result.delta_bundle.intent_lane
                if hasattr(result.delta_bundle, "intent_lane")
                else "SOFT"
            ),
        }
    ...
    async def reflect(
        ...
        if any(term in thought_lower for term in absolutist_terms):
            critique.append(
                "F7 Humility Violation: Avoid absolutist language. Use probabilistic reasoning (likely, suggests, potentially)."
            )
        ...
        if any(term in thought_lower for term in ontology_terms):
            critique.append(
                "F10 Ontology Violation: Agent must maintain tool ontology. Do not claim feelings, desires, or internal cognitive processes."
            )
        ...
        if word_count < 15:
            critique.append(
                f"F4 Clarity Violation: Thought is too brief ({word_count} words). Complexity requires entropy reduction through detailed analysis."
            )
        ...
        if "reality_search" not in thought_lower and current_depth > 1:
            if any(marker in thought_lower for marker in ["assume", "probably", "might be"]):
                critique.append(
                    "F2 Truth Warning: Ungrounded assumptions detected in revision. Suggest calling reality_search for grounding."
                )
        ...
        return {
            ...
            "verdict": status,
            "guidance": (
                "Thought validated. Proceed to output."
                if is_safe
                else f"Cycle {current_depth}: Revise thought to address violations."
            ),
    ...
    async def _execute_full(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "query": query,
            "insight": (
                delta_bundle.reasoning.conclusion if delta_bundle.reasoning else "Analysis complete"
            ),
            ...
            "verdict": (
                delta_bundle.vote.value
                if hasattr(delta_bundle.vote, "value")
                else str(delta_bundle.vote)
            ),
    ...
    async def _execute_atlas(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "knowledge_map": {
                "query": query,
                "intent": (
                    bundle.detected_intent if hasattr(bundle, "detected_intent") else "unknown"
                ),
                "hypotheses": bundle.hypotheses if hasattr(bundle, "hypotheses") else [],
                "confidence_range": (
                    [bundle.confidence_low, bundle.confidence_high]
                    if hasattr(bundle, "confidence_low")
                    else [0, result.get("truth_score", 0)]
                ),
    ...
    async def _execute_reason(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": result["status"],
            "reasoning": (
                bundle.reasoning.inference_steps
                if bundle.reasoning and hasattr(bundle.reasoning, "inference_steps")
...
__all__ = ["AGINeuralCore", "AGIKernel", "get_agi_core", "cleanup_expired_sessions"]
