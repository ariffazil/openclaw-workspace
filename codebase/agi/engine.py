...
class AGIEngineHardened:
    ...
    async def _think_path(self, path: Dict, sense_data: SenseData, exec_id: str) -> ThinkResult:
        ...
        base_confidences = {"conservative": 0.85, "exploratory": 0.75, "adversarial": 0.70}
        ...
        entropy_delta = sense_data.cumulative_delta_s * (
            0.9 if path["type"] == "conservative" else 0.95
        )
    ...
    async def _stage_333_forge(
        ...
        # F = ΔS + Ω₀·π⁻¹
        pi_inv = (
            1.0 / sense_data.precision.pi_likelihood
            if sense_data.precision.pi_likelihood > 0
            else 1.0
        )
        ...
        floor_scores = {
            ...
            "F7_humility": 1.0
            - (omega_0 - HUMILITY_BAND[0]) / (HUMILITY_BAND[1] - HUMILITY_BAND[0]),
    ...
    def _determine_vote(
        self, entropy_delta: float, omega_0: float, free_energy: float, policy: ActionPolicy
    ) -> EngineVote:
