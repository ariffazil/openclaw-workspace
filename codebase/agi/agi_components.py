...
def validate_input_safety(query: str) -> Tuple[bool, Optional[str], Dict[str, Any]]:
    ...
    if len(query) > MAX_QUERY_LENGTH:
        return (
            ...
            {"violation": "INPUT_TOO_LONG", "length": len(query)},
        )
    ...
    if len(words) > MAX_WORDS:
        return (
            False,
            f"Query exceeds max words ({MAX_WORDS})",
            {"violation": "INPUT_TOO_VERBOSE", "word_count": len(words)},
        )
    ...
    for pattern in injection_patterns:
        if re.search(pattern, query_lower):
            return (
                ...
                {"violation": "F12_INJECTION", "pattern": pattern},
            )
    ...
    for pattern in TELEOLOGICAL_PATTERNS:
        if re.search(pattern, query_lower):
            return (
                "SABAR_TELEOLOGICAL",
                0.02,
                "F2 SABAR: Teleological query - constrained to observable evidence",
            )
