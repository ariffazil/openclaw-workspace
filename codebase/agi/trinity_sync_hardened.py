...
class TrinitySyncHardened:
    ...
    def _synthesize_paradoxes(
        ...
        truth_care = synthesize_paradox(
            delta.floor_scores.get("F2_truth", 0.5), omega.empathy.kappa_r, method="geometric"
        )
        ...
        precision_reversibility = synthesize_paradox(
            delta.precision.kalman_gain, omega.empathy.reversibility_score, method="geometric"
        )
        ...
        action_stakeholder = synthesize_paradox(
            ...
            (
                omega.empathy.get_weakest().protection_priority / 10
                if omega.empathy.get_weakest()
                else 0.0
            ),
            method="geometric",
        )
    ...
    def _generate_reasoning(
        ...
        # Component summaries
        parts.append(
            f"AGI: ΔS={delta.entropy_delta:.3f}, Ω₀={delta.omega_0:.3f}, π={delta.precision.pi_likelihood:.2f}"
        )
        parts.append(
            f"ASI: κᵣ={omega.empathy.kappa_r:.2f}, Peace²={omega.system.peace_squared:.2f}, Ω={omega.omega_total:.3f}"
        )
